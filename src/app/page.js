    ...
    <div className="min-h-screen bg-black text-zinc-100 font-sans selection:bg-orange-500 selection:text-white relative overflow-hidden">
      ...
      <header className="border-b border-zinc-900 bg-black/60 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-9 w-9 flex items-center justify-center transition-transform hover:scale-105">
          ...
          <nav className="flex items-center gap-4">
            {loading ? (
              ...
              <div className="flex items-center gap-3">
                ...
                <Link
                  href={currentUser.role === 'superAdmin' || currentUser.email.toLowerCase() === 'admin' ? '/superadmin' : (currentUser.orgId ? `/${currentUser.orgId}/${currentUser.userId || currentUser.id}` : '#')}
                  onClick={(e) => {
                    if (currentUser.role !== 'superAdmin' && !currentUser.orgId) {
                      e.preventDefault();
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 space-y-20 relative">
        <div className="text-center max-w-4xl mx-auto space-y-8">
          <div className="flex justify-center mb-6 relative">
            ...
            <Image 
              ...
              width={160} 
              height={160} 
              className="object-contain drop-shadow-[0_0_30px_rgba(249,115,22,0.5)] relative z-10 hover:scale-105 transition-transform duration-500" 
              priority
            />
          </div>
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-orange-950/40 border border-orange-500/40 text-orange-400 text-sm font-semibold shadow-[0_0_20px_rgba(234,88,12,0.1)]">
        ...
        <div className="bg-gradient-to-r from-orange-950/20 via-zinc-950 to-orange-950/10 border border-zinc-900 rounded-3xl p-8 sm:p-12 text-center space-y-6 relative overflow-hidden">
          ...
          <div className="pt-2">
            <Link
              href={currentUser ? (currentUser.role === 'superAdmin' || currentUser.email.toLowerCase() === 'admin' ? '/superadmin' : (currentUser.orgId ? `/${currentUser.orgId}/${currentUser.userId || currentUser.id}` : '#')) : "/register"}
              onClick={(e) => {
