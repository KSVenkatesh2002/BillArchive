    ...
    <SectionCard>
      <div>
        ...
        <p className="text-[10.5px] text-zinc-405 mt-1">
          Add, manage, and assign credentials for employees/workers under your
          ...
          {loadingUsers ? (
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
              {orgUsers.map((u) => (
                <div
                  key={u.id}
                  className="p-3 bg-black border border-zinc-800 rounded-xl flex justify-between items-center"
                >
                  <div>
                    <p className="text-xs font-bold text-white">{u.name}</p>
                    <p className="text-[10.5px] text-orange-400 font-mono">
                      {u.email}
                    </p>
                  </div>
                  <span
